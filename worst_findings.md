...
## 1. Undo and redo mutate history before the filesystem operation succeeds

- References: `src/CullendulaFileSystemHandler.cpp:226`, `src/CullendulaFileSystemHandler.cpp:251`, `src/CullendulaMainWindow.cpp:474`, `src/CullendulaMainWindow.cpp:485`
- Problem: `undo()` and `redo()` pop from the logical history first and only then attempt the filesystem rename. If the rename fails, the method returns `false`, but the stack has already advanced. The main window also does not surface any actionable undo/redo failure to the user.
- Impact: this is a correctness problem in the core safety mechanism of a file-moving tool. After a failed undo/redo, the in-memory history can diverge from the filesystem state, and the user receives weak or no recovery guidance.

## 2. Loading a new path destroys the current session before the new path is validated
...
## 3. The pipeline is not a pure verification step; it rewrites source files and still exits successfully
...
## 4. Drag-and-drop path handling is brittle and manually platform-specific
...
## 5. A missing current file leaves the main window in an inconsistent UI state
...
## 6. Test registration is monolithic, so CTest signal and parallelism are much weaker than they look
...
## 7. Release/version information is duplicated across several manual edit surfaces
...
## 8. The production bootstrap forces `QT_QPA_PLATFORM=offscreen` when the environment is empty
...
## 9. The bootstrap tests mutate process-global environment state and do not restore it
...
## 10. The project still treats branch coverage as informational despite it being the obvious weak spot
